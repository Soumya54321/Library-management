...
def setevent_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            event=form.save()
            students=list(models.StudentExtra.objects.all())
            li = []
            i=0
            for l in students:
                t=(students[i].get_email)
                i=i+1
                li.append(t)
            #Send mail
            send_mail(
                'New Events added',
                'There has been a new event for you on Central e-Librarian. Please login to the portal to check the detais of the event.',
                'soumyadeeppaul09@gmail.com',
                li,
                fail_silently = False
            )
            return render(request,'library/eventadded.html')
        else:
            return render(request,'library/setevent.html',{'form':form})
    else:
        return render(request,'library/setevent.html',{'form':form})


def viewevent_view(request):
    events=models.Events.objects.all()
    return render(request,'library/viewevent.html',{'events':events})
